...
| --- | ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| C1  | DTOs                     | `@IsEnum(['amarilla','roja'])` es incorrecto: `IsEnum` espera un objeto enum, no un array. Debe ser `@IsIn` o un enum propio.                                                                                                                                         | **No aplica**: el código no usa `class-validator` ni DTOs en ningún punto — la carga de eventos vive en flows (`ContextoFlujo`/`Paso`) que parsean texto libre, no en un body validado por decoradores. |
...
| M3  | Si Redis se caía, el bot moría. Ahora arranca igual y degrada a Postgres.                                                                                                                                               | **Resuelto y verificado**: sin Redis el proceso sigue vivo, y `/health` lo reporta como `no_configurado` con `estado: ok` — no como una caída. |
| M4  | "MVP del partido" no tenía regla definida. **Decidido**: sistema de puntos por evento (ver abajo).                                                                                                                      | **Resuelto**: `src/eventos/puntaje.ts`, integrado en el resumen automático (`resumen.service.ts`). |
